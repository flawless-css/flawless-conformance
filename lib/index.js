...
/**
 * Conformance
 * The conformance runner manages tasks that operate during the parse of
 * the less. The runner collates output and allows maintains parse reports.
 * @class
 */
export default class Conformance extends Transformer {
    ...
    constructor( opts ) {
        ...
        // Compile mode overrides stream mode, this is enforced also in the transformer class
        if ( this.opts.compiling ) {
            this.opts.streaming = false
        }

        // Mixin the log object
        core.Object.define( this, Logger.prototype )
    ...
    parse( css, callback ) {
        less.parse( css, {
            ...
            // Generate parseTree if compiling
            var parsed = null
            if ( this.opts.compiling && !this.opts.streaming ) {
                var parseTree = new less.ParseTree( root, imports )
                parsed = parseTree.toCSS( {} ).css
            }
    ...
    _processNode( node ) {
        ...
        this.emit( PARSE.NOT_RECOGNISED, {
            raw: node
        })

        // Doesnt yet parse node.type === 'Directive', stuff like @font-face and @keyframes
    }
